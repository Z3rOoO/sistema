...
class TutorModel {
    ...
    static async listarTodos() {
        ...
        // TODO: Implementar a busca de todos os tutores
        // Dica: use a função read('clientes')
    }

    // Buscar cliente por ID
    static async buscarPorId(id) {
        const rows = await read('tutor', `id = ${id}`);
        return rows[0] || null;
    ...
    static async atualizar(id, dados) {
        return await update('tutor', dados, `id = ${id}`);
    ...
    static async excluir(id) {
         return await deleteRecord('tutor', `id = ${id}`);
